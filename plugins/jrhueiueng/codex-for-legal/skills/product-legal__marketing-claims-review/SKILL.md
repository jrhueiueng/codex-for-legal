...
1. Load `~/.codex/plugins/config/codex-for-legal/product-legal/CLAUDE.md` → Marketing claims standards.
...
Read `~/.codex/plugins/config/codex-for-legal/product-legal/CLAUDE.md` → `## Marketing claims`:
...
Per `~/.codex/plugins/config/codex-for-legal/product-legal/CLAUDE.md` — if comparative claims are "never," flag all of them. If "allowed with substantiation," check for the substantiation.
...
Prepend the work-product header from `~/.codex/plugins/config/codex-for-legal/product-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
> **Before emitting "Ready to ship: Yes" (i.e., approving a claim for external use / publication):** Read `## Who's using this` in `~/.codex/plugins/config/codex-for-legal/product-legal/CLAUDE.md`. If the Role is Non-lawyer:
>
