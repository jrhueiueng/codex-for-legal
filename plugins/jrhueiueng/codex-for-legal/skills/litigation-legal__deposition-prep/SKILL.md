...
1. Load `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md` → case theory, key facts.
...
`~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md` → case theory (theory, pivot fact, key facts for/against), eDiscovery platform.

**Conflicts gate — unbypassable.** Before building an outline, check `~/.codex/plugins/config/codex-for-legal/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
- Facts from `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md` → key facts for us, that this witness can establish
