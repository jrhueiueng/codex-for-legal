...
1. Load `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` → clinic profile, semester dates, supervision style.
2. Load `~/.codex/plugins/config/codex-for-legal/legal-clinic/deadlines.yaml` and `~/.codex/plugins/config/codex-for-legal/legal-clinic/client-comms/[case-id]/log.md` per case.
...
5. Generate per-case handoff memo → `~/.codex/plugins/config/codex-for-legal/legal-clinic/handoffs/[semester]/[case_id].md`.
6. Generate cohort summary → `~/.codex/plugins/config/codex-for-legal/legal-clinic/handoffs/[semester]/_summary.md`.
...
- `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` → clinic profile, semester, practice areas, supervision style
- `~/.codex/plugins/config/codex-for-legal/legal-clinic/deadlines.yaml` → all active deadlines, grouped by case
- `~/.codex/plugins/config/codex-for-legal/legal-clinic/client-comms/[case-id]/log.md` (per case) → communications history
...
- Pull all active cases (from intake records + `~/.codex/plugins/config/codex-for-legal/legal-clinic/deadlines.yaml` case_ids + client-comms folders)
...
*Pulled from `~/.codex/plugins/config/codex-for-legal/legal-clinic/deadlines.yaml`. Incoming student's first job is to confirm these are accurate and owned.*
...
*From `~/.codex/plugins/config/codex-for-legal/legal-clinic/client-comms/[case-id]/log.md`. Three-line summary here; incoming student reads the full log.*
...
After all per-case memos, produce `~/.codex/plugins/config/codex-for-legal/legal-clinic/handoffs/[semester]/_summary.md`:
...
Closing a case or transitioning it to a new student is a consequential action. The gate is the supervision workflow in `## Supervision style` in `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md`, reinforced by the Part 0 role check confirming a licensed supervising attorney owns the setup. Case-closing memos always get professor sign-off before the case is marked closed in the handoff document, regardless of supervision-style choice.

Per `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` supervision style:
...
Once reviewed, handoff memos live at `~/.codex/plugins/config/codex-for-legal/legal-clinic/handoffs/[semester]/[case_id].md`. The incoming student reads them during their `/ramp` run at the start of next semester — `/ramp` should surface the memos for cases the new student is assigned.

## Integration

- **`/ramp`:** at the start of next semester, reads `~/.codex/plugins/config/codex-for-legal/legal-clinic/handoffs/[most-recent-semester]/` and surfaces per-case memos for the cases each new student is taking on.
