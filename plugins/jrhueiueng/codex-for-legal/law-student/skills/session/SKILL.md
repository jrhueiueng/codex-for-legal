...
2. Load `~/.codex/plugins/config/codex-for-legal/law-student/CLAUDE.md` → jurisdiction, exam format, weak subjects.
3. Load `~/.codex/plugins/config/codex-for-legal/law-student/study-plan.yaml` if it exists. Read `session_history` for this subject to weight subtopics toward where the student has been weak.
...
6. At session end, write session results:
   ...
   - If not: write to `~/.codex/plugins/config/codex-for-legal/law-student/session-history.yaml`.
7. Report:
