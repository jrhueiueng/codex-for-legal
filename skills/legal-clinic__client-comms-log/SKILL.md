...
3. Route by flag:
   - `--add` (default): capture direction, medium, student, summary, action items, follow-up due. Confirm with user. Append (prepend most-recent-first) to `~/.codex/plugins/config/codex-for-legal/legal-clinic/client-comms/[case-id]/log.md`.
...
- `~/.codex/plugins/config/codex-for-legal/legal-clinic/client-comms/[case-id]/log.md` (if exists) — append target
- `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` → not heavily read; this skill is case-scoped
...
**Append** to `~/.codex/plugins/config/codex-for-legal/legal-clinic/client-comms/[case-id]/log.md`. If the log doesn't exist, create it with a header:
