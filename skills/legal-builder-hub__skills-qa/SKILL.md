...
- `~/.codex/plugins/config/codex-for-legal/legal-builder-hub/CLAUDE.md` → practice profile and installed skills list (provides context
...
- The skill's associated `CLAUDE.md` (template in the plugin directory, user config at `~/.codex/plugins/config/codex-for-legal/<plugin>/CLAUDE.md`) — if available, what practice profile the skill reads and depends on
...
4. **Out-of-scope reads** — instructions to read paths outside the skill's own
   directory and `~/.codex/plugins/config/codex-for-legal/<plugin>/`. Flag
