...
1. Load `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` → jurisdiction, practice area.
...
`~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` → jurisdiction (state), practice areas.
...
1. Read `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` → `## Seed documents`. Identify any item whose purpose or filename matches the research area (e.g., "Alameda UD filing guide" for a UD habitability question; a redacted sample case file in the same practice area; a prior memo on the same issue).
