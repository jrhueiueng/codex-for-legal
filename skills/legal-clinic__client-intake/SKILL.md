...
1. Load `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` → practice areas, intake templates, supervision style, flag triggers.
...
Each practice area asks different questions. Use the template from `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` for this area. Defaults if none provided:
...
Per whatever conflict-check process `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` describes. At minimum:
...
Per `~/.codex/plugins/config/codex-for-legal/legal-clinic/CLAUDE.md` supervision style and flag triggers. If formal queue or configurable flags are enabled, and a trigger is present (deadline mentioned, DV indicator, immigration status at issue, etc.), note the flag.
