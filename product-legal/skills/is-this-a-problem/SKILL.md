...
1. Load `~/.codex/plugins/config/codex-for-legal/product-legal/CLAUDE.md` → Risk calibration.
...
Read `~/.codex/plugins/config/codex-for-legal/product-legal/CLAUDE.md` → `## Risk calibration`. The whole point of this skill is pattern-matching against that table.
...
| "Can we use [AI vendor / LLM] for this?" | Vendor AI terms may permit training on inputs; use case may need an AIA — flag and route to `/product-legal:use-case-triage` | "Is there an AI addendum? What data goes into the model?" |
| "Can we add AI to this feature?" | May be a new use case not in the registry; may trigger AIA requirement — flag and route to `/product-legal:use-case-triage` | "What does the AI do — assistive or automated? Who does it act on?" |
...
| "We're just fine-tuning on our data" | Training data rights, output IP, and vendor obligations all change — flag and route to `/product-legal:vendor-ai-review` | "What's in the training data? Is any of it customer or employee data?" |
...
Slack triage replies are internal legal advice. If the reply is being pasted into a ticket, document, or channel that's broadly shared with non-legal, prepend the work-product header from `~/.codex/plugins/config/codex-for-legal/product-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`):
...
ships. Takes a day. Want me to run `/product-legal:use-case-triage` now?
```
