...
description: >
  ...
  Runs per the cadence in ~/.codex/plugins/config/codex-for-legal/regulatory-legal/CLAUDE.md. Filters by materiality threshold so the
...
Per `~/.codex/plugins/config/codex-for-legal/regulatory-legal/CLAUDE.md` → Feed configuration → Check cadence. Default weekly; daily if the regulatory environment is active.

## What it does

1. Read `~/.codex/plugins/config/codex-for-legal/regulatory-legal/CLAUDE.md` → watchlist, materiality threshold.
