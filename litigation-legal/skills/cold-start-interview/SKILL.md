...
1. Check `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md`. If already populated and no `--redo`, ask before overwriting.
...
6. Write `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md`. Date the footer.
7. Confirm with the user before finalizing: "Here's what I captured — anything wrong?"

## Flags

- `--redo` — re-run the full interview and overwrite `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md` without re-running the full interview. Use after setting up a new connector (DMS, document storage, Gmail, scheduled-tasks, CLM).
...
Read `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md`:
...
> **`litigation-legal` is for people who work litigation — managing a portfolio of matters in-house, drafting briefs and doing discovery at a firm, or both as a solo practitioner.** Not your area? `/litigation-legal:related-skills-surfacer`.
>
...
> "Your practice profile is at `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
