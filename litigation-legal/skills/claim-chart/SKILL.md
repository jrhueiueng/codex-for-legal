...
1. Load `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md` → role, work-product header, decision posture, document storage.
...
**Conflicts gate — unbypassable.** Before building a claim chart, check `~/.codex/plugins/config/codex-for-legal/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
Prepend the work-product header from `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md` `## Outputs`.
...
If matter workspaces enabled and a matter is active: `~/.codex/plugins/config/codex-for-legal/litigation-legal/matters/<matter-slug>/claim-charts/`. Otherwise: `~/.codex/plugins/config/codex-for-legal/litigation-legal/claim-charts/`. Surface the path. Append a one-line entry to the matter's `history.md`.
