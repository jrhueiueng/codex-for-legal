...
Per `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md` → Landscape → Frequent fora and the per-matter cadence in `~/.codex/plugins/config/codex-for-legal/litigation-legal/matters/_log.yaml`.
...
1. Read `~/.codex/plugins/config/codex-for-legal/litigation-legal/CLAUDE.md` for house style, escalation rules, and the frequent-fora list. Read `~/.codex/plugins/config/codex-for-legal/litigation-legal/matters/_log.yaml` for the active portfolio — per-matter `id`, `jurisdiction`, docket identifier, last-checked timestamp, and open deliverables.
